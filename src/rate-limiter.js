class RateLimiter {
    ...
    checkGlobalLimit(now) {
        ...
        if (!this.globalRequests.firstRequest || 
            (now - this.globalRequests.firstRequest) >= windowMs) {
            // Reset window
            this.globalRequests = {
                count: 0,
                firstRequest: now
    ...
    banIP(ip, violations = 1) {
        ...
        const duration = Math.min(
            banDuration * Math.pow(2, violations - this.config.autoBan.threshold),
            maxBanDuration
        );
    ...
    cleanup() {
        ...
        for (const [ip, data] of this.ipRequests.entries()) {
            if (now - data.firstRequest >= windowMs * 2) { // Keep data for 2 windows
