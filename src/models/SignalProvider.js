...
signalProviderSchema.index({ communityId: 1, providerId: 1 }, { unique: true, sparse: true }); // Unique per community
signalProviderSchema.index({ communityId: 1, isActive: 1 }); // Tenant-scoped active providers
...
signalProviderSchema.methods.recordTradeResult = async function (
  success,
  profitLoss = 0,
  hitStopLoss = false,
  hitTakeProfit = false
...
signalProviderSchema.methods.addReview = async function (userId, rating, comment) {
  ...
  const existingReview = this.reviews.find(review => review.userId && review.userId.toString() === userId.toString());
...
signalProviderSchema.plugin(tenantScopingPlugin);
