...
async function waitForWebSocketConnection(page, timeout = WEBSOCKET_TIMEOUT) {
  await page.waitForFunction(() => window.socket && window.socket.connected === true, { timeout });
}
...
test.describe('WebSocket Performance Monitoring', () => {
  ...
  test('should handle high message volume without performance degradation', async ({ page }) => {
    ...
    console.log(
      `✅ High volume handling verified (${messageCount} messages in ${totalTime}ms, ${avgTimePerMessage.toFixed(2)}ms avg)`
    );
  });
  ...
  test('should handle rapid connect/disconnect cycles', async ({ page }) => {
    ...
    for (let i = 0; i < cycles; i++) {
      ...
      await page.waitForFunction(() => !window.socket || window.socket.connected === false, { timeout: 2000 });
