...
class SecureAutoSetup {
  ...
  containsSensitiveData(message) {
    const sensitivePatterns = [/token/i, /key/i, /password/i, /secret/i, /credential/i];
  ...
  async waitForElement(selector, options = {}) {
    const { timeout = 30000, visible = true, retry = true } = options;
  ...
  async fillFormSecurely(selector, value, options = {}) {
    const { clearFirst = true, validate = true, mask = false } = options;
    ...
    try {
      ...
      try {
        ...
        if (isLoginVisible) {
          ...
          const captchaPresent = await this.page
            .locator('.h-captcha, .g-recaptcha, [data-hcaptcha-theme]')
      ...
      try {
        ...
        for (const selector of tokenSelectors) {
          try {
            ...
            if ((botToken && botToken.startsWith('MTA')) || botToken.startsWith('ODc') || botToken.startsWith('MTc'))
              break;
          } catch (error) {
            continue;
          }
      ...
      try {
        ...
        const inviteUrl =
          (await this.page.inputValue('input[readonly]:last-child, textarea[readonly]')) ||
...
╚══════════════════════════════════════════════════════════════╝
    ...
    try {
      ...
      if (failureCount > 0) {
        console.log(chalk.yellow('\nFailed Services:'));
        this.setupProgress.filter(p => p.status === 'failed').forEach(p => console.log(`  • ${p.service}: ${p.error}`));
      }
    ...
    try {
      ...
      while (retries < this.maxRetries) {
        try {
          ...
          // Handle potential CAPTCHA
          const captchaPresent = await this.page
            .isVisible('[data-testid="captcha"]', { timeout: 3000 })
            .catch(() => false);
          if (captchaPresent) {
          ...
          const twoFactorPresent = await this.page
            .isVisible('input[placeholder*="code"], input[name*="token"]', { timeout: 5000 })
            .catch(() => false);
          if (twoFactorPresent) {
      ...
      await this.waitForElement('[data-testid="api-keys-container"], .api-keys, [aria-label*="API"]', {
        timeout: 15000
      });
      ...
      if (!secretKey || !secretKey.startsWith('sk_test_')) {
        ...
        const revealButtons = await this.page.$$(
          'button:has-text("Reveal"), button[aria-label*="Reveal"], button[data-testid="reveal"]'
        );
        ...
        if (webhookElements.length > 0) {
          ...
          for (const selector of webhookSecretSelectors) {
            try {
              ...
              if (secretElement) {
                const webhookSecret =
                  (await secretElement.textContent()) || (await secretElement.getAttribute('value'));
    ...
    try {
      ...
      while (retries < this.maxRetries) {
        try {
          ...
          // Handle potential CAPTCHA
          const captchaPresent = await this.page
            .isVisible('[data-testid="captcha"], .captcha, #captcha', { timeout: 5000 })
            .catch(() => false);
          if (captchaPresent) {
      ...
      if (setupOption.option === 'atlas') {
        ...
        while (retries < this.maxRetries) {
          try {
            ...
            // Handle potential CAPTCHA
            const captchaPresent = await this.page
              .isVisible('[data-testid="captcha"], .captcha, .recaptcha', { timeout: 5000 })
              .catch(() => false);
            if (captchaPresent) {
        ...
        while (retries < this.maxRetries) {
          try {
            ...
            if (apiKeyElements.length > 0) {
              ...
              const keyValueSelectors = ['code:has-text("SG.")', 'input[readonly][value*="SG."]', '.api-key-value'];
